...
本仓库包含三个完整的R语言地理空间数据分析教程：

1. **[R区域统计教程](./readme.md)** - 基于TIF/NetCDF栅格数据计算Shapefile区域的zonal mean（本文档）
2. **[R点位缓冲区栅格提取教程](./R_点位缓冲区栅格提取教程.md)** - 从CSV读入点位坐标，计算球面距离缓冲区内的栅格平均值/IDW加权值
3. **[R点对点匹配与加权聚合教程](./R_点对点匹配与加权聚合教程.md)** - 企业-监测站点对点匹配、球面距离计算、逆距离加权聚合
